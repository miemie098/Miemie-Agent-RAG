...
flowchart LR
    ...
    subgraph Retrieval[多阶段检索 Pipeline]
        ...
        Dense --> Fusion[线性加权融合<br/>α=0.6 Dense 稍偏重]
        BM25 --> Fusion
...
### 融合策略对比结论（2026-07-23，ONNX INT8，n=9，10 配置消融）

| 排名 | 策略 | 平均分 | P50 | P99 |
|---|---|---|---|---|
| [BEST] #1 | **Linear α=0.6** | **7.86** | 17.8s | 38.2s |
| #2 | Linear α=0.8 | 7.54 | 21.2s | 33.8s |
| #3 | Linear α=0.4 | 7.51 | 16.7s | 32.1s |
| #4 | RRF (k=60) | 7.40 | 17.0s | 35.4s |
| #5 | Linear α=0.3 | 7.40 | 18.9s | 37.3s |
| #6 | Linear α=0.2 | 7.37 | 16.9s | 31.7s |
| #7 | Linear α=0.9 | 7.34 | 19.1s | 41.6s |
| #8 | Linear α=0.1 | 7.31 | 15.4s | 29.2s |
| #9 | Linear α=0.7 | 7.29 | 17.0s | 36.0s |
| #10 | Linear α=0.5 | 7.16 | 18.0s | 36.7s |

**精度-延迟 trade-off**：

```
8.0 ┤                         ● α=0.6 (7.86, 17.8s)  ← 帕累托最优
7.8 ┤
7.6 ┤     ● α=0.8
7.5 ┤         ● α=0.4                    ● RRF
7.4 ┤   ● α=0.1    ● α=0.3  ● α=0.2
7.3 ┤           ● α=0.9    ● α=0.7
7.2 ┤
7.1 ┤                               ● α=0.5  ← U 型谷底
     └────┬────┬────┬────┬────┬────┬────
        15    16    17    18    19    20    21   P50 (s)
```

**结论**：
- α=0.6（Dense 稍偏重）帕累托最优，精度最高且延迟适中
- α=0.5 等权是 U 型谷底，验证了两路信号的互补性——绝对等权反而丢失有效召回
- RRF 不再是唯一最优，线性加权在合适 α 下可同时获得更高精度和更低延迟
- 原始结论（ONNX 优化前）受 PyTorch 推理噪声干扰，优化后数据更可信
